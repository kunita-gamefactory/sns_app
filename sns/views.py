    ...
    if request.method == 'POST':
        ...
        if request.POST['mode'] == '__friends_form__':
            ...
            for item in fds:
                ...
                vlist.append(item.user.username)
            # メッセージを設定
            messages.success(request, ' チェックされた友達を' + \
                    sel_group + 'に登録しました。')
            # フォームの用意
...
def add(request):
    ...
    if add_user == request.user:
        messages.info(request, "自分自身を友達に追加することは\
                できません。")
    ...
    messages.success(request, add_user.username + ' を追加しました！\
        コミュニティページに移動して、追加した友達をメンバーに設定して下さい。')
    return redirect(to='/sns')
...
def creategroup(request):
    ...
    gp.save()
    messages.info(request, '新しいコミュニティを作成しました。')
...
def good(request, good_id):
    ...
    # ゼロより大きければ既にgood済み
    if is_good > 0:
        messages.success(request, '気持ちは嬉しいですが、投稿には「ナイス♡」済みです。')
        return redirect(to='/sns')
    ...
    good.save()
    # メッセージを設定
    messages.success(request, '投稿に「ナイス♡」しました！')
    return redirect(to='/sns')
...
def signupview(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return render(request, "sns/signup.html", {"error": "このユーザーIDは既に登録されています"})
    else:
